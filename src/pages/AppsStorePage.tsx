          ...
          {isLoading ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} dir="rtl" className="w-full">
              ...
              <AnimatePresence mode="wait">
                <TabsContent key="tab-social" value="social" className="mt-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {socialApps.map((app, i) => <AppCard key={app.id || `social-${i}`} app={app} index={i} />)}
                    {socialApps.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">لا توجد تطبيقات في هذا القسم</p>}
                  </div>
                </TabsContent>

                <TabsContent key="tab-tools" value="tools" className="mt-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {toolsApps.map((app, i) => <AppCard key={app.id || `tools-${i}`} app={app} index={i} />)}
                    {toolsApps.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">لا توجد تطبيقات في هذا القسم</p>}
                  </div>
                </TabsContent>

                <TabsContent key="tab-design" value="design" className="mt-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {designApps.map((app, i) => <AppCard key={app.id || `design-${i}`} app={app} index={i} />)}
                    {designApps.length === 0 && <p className="text-center text-muted-foreground col-span-2 py-8">لا توجد تطبيقات في هذا القسم</p>}
                  </div>
                </TabsContent>

                <TabsContent key="tab-wifi" value="wifi" className="mt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {wifiApps.map((app, i) => <AppCard key={app.id || `wifi-${i}`} app={app} index={i} />)}
